...
void CMWEditView::OnEditOutputrecinfo() {
	...
	for (Index = 0; Index < pArray->GetSize(); Index++) {
		SystemLog.Printf ("ID = '%s' (%s, %4.4s), %s", (*pArray)[Index]->pRecord->GetID(),
...
void CMWEditView::OnTestGettype() {
	...
	esmrecinfo_t *pRecInfo;
	bool Result;
	Result = InputDlg.DoModal(&Buffer, _T("Enter record ID to get the type of:"),
	                          _T("Enter Record ID"));
	...
	if (pRecInfo == NULL) {
		...
		Buffer.Format(_T("Item type is: %s (%4.4s)"), pRecInfo->pRecord->GetItemType(),
